...
- Buttons, checkboxes and radio buttons are `ReleaseClickButton` / `ReleaseClickCheckBox` / `ReleaseClickRadioButton`, never the WinForms types they derive from — in the designer as well as in code. `ButtonBase` raises `Click` only while `WindowFromPoint` at the release point still answers with the control's own handle, so any window overlapping that pixel (a tooltip, above all — measured: 20 of 20 clicks lost) takes the click silently while the control still paints its press. They repair that failure and no other: the hit test is re-run before a click is manufactured, so a click WinForms withheld for a reason of its own (cancelled validation, its press/capture state) stays withheld. A menu is then opened through `DropDownMenu`, which posts the show clear of the mouse message and guards it against the focus change opening it causes.
