...
- **Live Spectrum's state is a `LiveSpectrumSession`**: the run, the accumulation it holds after a stop or the capture loaded in its place, the peak-hold envelope and the next run's frozen inputs. `LiveSpectrumDisplay` turns the options and one read of the analyzer (`LiveCaptureSetup`) into what a build draws, and `LiveSpectrumCurves` draws it as points, the capture document and the overlays' raw RTA included. `LiveSpectrumController` is the plot binding and a mode view beside `AnalyzerPlot`; the form's record button, Save, calibration read-out and settings panel follow the session's `Changed`. `PlotModelFactory` does not read the live analyzer. See `docs/tech/live-spectrum.md#code-map`. The settings panel's own state is a `LiveSpectrumSettingsSession`, read by `LiveSpectrumSettingsChoices`, `LiveSpectrumSettingsLook` and `LiveSpectrumSettingsToolTips`; see `docs/tech/live-spectrum.md#settings-panel-code-map`.
- **`Options/`** holds one settings panel per mode (`FROptions`, `IROpt`, `GDOpt`, ...), docked into the shell via `Shell/DockedModeSettingsHost`. The analysis modes' panels keep their fields in sessions (`FrequencyResponseSettingsSession`, `GatedAnalysisSettingsSession` for Phase and Group Delay, `WaterfallSettingsSession`, `ImpulseViewSettingsSession`) with shared parts (`TukeyFades`, `WindowModeChoice`, `GateFields`, `SmoothingPresetOptions`) that the settings file reads too; readers such as `FrequencyResponseSplChoice`, `PhaseDetrendEstimate` and `ImpulseBandCentres` turn them into what the panels show. A panel follows the open measurement through `ModeSettingsForm` and announces the user's edits (`IUserEditedSettings`), which alone apply. See `docs/tech/mode-settings.md#code-map`.
- **The EQ Wizard's state is an `EqWizardSession`**: the source and how it is read, the target and its level, the processor, the Auto Tune settings, the bank with its undo history (`EqWizardBank`), the phase gate and the Virtual DSP handoff, each value held as its field shows it (`EqWizardLimits`). Readers turn it into the source curve, the render set and statistics, the fit's inputs and the plot (`EqWizardSourceCurve`, `EqWizardRender`, `EqWizardFit`, `EqWizardPlot`); `EqWizardPanel` binds the controls and runs the dialogs and the fit. See `docs/tech/eq-auto-tuner.md#code-map`.
- **Time Alignment's state is a `TimeAlignmentSession`**: what is read (the open measurement and the compare selection), the band options, the read schedule and the last Auto band. `TimeAlignmentSources`, `TimeAlignmentBand` and `TimeAlignmentRead` turn it into a read; `TimeAlignmentRecommendation`, `TimeAlignmentReport` and `TimeAlignmentPreviews` turn the read's outcome into the table's verdict, the status text and the two previews. `TimeAlignmentPanelController` binds the controls and runs reads off the UI thread. See `docs/tech/junction-phase-and-group-placement.md#time-alignment-code-map`.
- **Record Settings' state is a `RecordSettingsSession`**: the audio route, the sweep, the protective high-pass and the calibrations, each field held as its control shows it (`RecordChoice`, `RecordNumber`), with the rules that move one field when another does; the hardware is read through `IRecordDevices`. `RecordDeviceStatus`, `RecordCalibrations`, `RecordArrayInputs`, `SweepBandPreview` and `RecordHighPass` read it into what the panel shows, and `RecordSettingsApply` checks the route and hands it to the engine on Apply. `MeasurementOptions` binds the controls and runs the dialogs. See `docs/tech/sweep-measurement.md#record-settings-code-map`. Its dialogs keep their state in `MicrophoneCalibrationsSession`, `AngleCalibrationSession`, `ArrayMicrophonesSession`, `SplCalibrationSession` and `RecordedSweepChannelChoice`, read by `MicrophoneCalibrationRows`, `AngleCalibrationPreview`, `ArrayMicrophoneRows` and `SplCalibrationReport`; which array channels a run records is `ArrayChannelRules` alone. See `docs/tech/sweep-measurement.md#calibration-dialogs-code-map`.
- **`Tools/`** contains the larger feature panels: EQ Wizard, Signal Generator, Virtual DSP (`VirtualCrossoverPanel` + project file persistence), and PDF tuning-sheet export (PDFsharp/MigraDoc). The EQ Wizard is self-contained: its session owns its source, uses a mode-local target curve and persists through `MeasurementSettingsFile.EqWizard`. It picks that source itself — an impulse response (file or history), a captured overlay slot, or a text curve — through `EqWizardSourceResolver`, which reads overlay slot FILES and history results as one-time imports. Keep it that way: the panel must not reach into the live `OverlaySession` or the current measurement, and an imported curve is a snapshot with no link back to what it came from.
...
- **The crossover wizard's state is an `AutoSetupWizardSession`**: the channels in chain order with their driver types, what the user set on each junction (kept by pair), the families, the system band and the options. `AutoSetupWizardPlan`, `AutoSetupWizardFit`, `AutoSetupWizardReport` and `AutoSetupWizardChainOrder` read it into the search's inputs, the fit and preview, the texts and the order verdicts; `VirtualCrossoverAutoSetupDialog` binds the controls. Tune junction's Apply and its one step of Undo live in `VirtualCrossoverJunctionTuneApply` over the `VirtualCrossoverSession`, its search and verdict in `VirtualCrossoverJunctionTuneSearch`. See `docs/tech/virtual-dsp-panel.md#crossover-wizard-code-map`.
- **Virtual DSP's dialogs and hand-offs have owners of their own.** Tune junction's question is a `VirtualCrossoverJunctionTuneQuestion`, the processor dialog's choice a `DspProcessorSession` (with `DspProcessorStatus` and `DspProcessorApply`), the Gate dialog's readings `VirtualCrossoverGateEstimate`; what a channel hands the EQ Wizard and whether the bank may come back is `VirtualCrossoverEqHandoff`, and an AI import once its review is answered is `AgentImportRunner`, which reaches the panel only through `IAgentImportHost`. The target level's one owner is the project, and the ranges a reply can state too are `VirtualCrossoverLimits`. See `docs/tech/virtual-dsp-panel.md#code-map`.
- **The audition's state is a `VirtualCrossoverAuditionSession`**: the track and the output with the consent to replace it, the calibration, cabin and magnitude choices, the report and the render in flight; what the next dialog restores is a `VirtualCrossoverAuditionMemory`. `VirtualCrossoverAuditionBudget`, `VirtualCrossoverAuditionCalibration`, `VirtualCrossoverAuditionRender` and `VirtualCrossoverAuditionReport` read it into the refusals, the calibration a render carries, the render itself and the report; `VirtualCrossoverAuditionDialog` binds the controls. A channel block's read-outs come from `VirtualCrossoverChannel*Readout` readers over the values it holds. See `docs/tech/virtual-dsp-panel.md#audition-code-map`.
- **The FIR Constructor's state is a `FirConstructorSession`**: the kernel shown (a design's, or a bare one any edit replaces), the Virtual DSP side it edits with the standalone work set aside, and the rebuild in flight. `FirConstructorRender`, `FirConstructorReadout`, `FirConstructorAvailability`, `FirConstructorChoices` and `FirConstructorExport` read it into the plots, the texts, what the fields and buttons take, the pick lists and the exported file; `FirConstructorPanel` binds the controls. See `docs/tech/dsp-fir-crossover-design.md#constructor-code-map`.
...
- **The overlay slots' state is an `OverlaySession`**: each slot's content as an immutable `OverlaySlotState` (what its file stores), whether it shows, and the rules for capture, import, calculated overlays, targets and the deferred offset save. Readers draw it (`OverlayCurves`, `OverlaySeries`) and read the plot it sits on (`OverlayPlotSources`); `OverlayPanel` and the `OverlaySlotView` partials bind the controls and run the dialogs. See `docs/tech/overlays.md#code-map`. **`History/`** keeps measurement results with per-entry working state.
...
- **State gets a UI-free owner before features are written against it.** Once a second feature reads a tool's data,
  the data moves into a plain type (`VirtualCrossoverSession`, `AnalyzerDocument`, `LiveSpectrumSession`, `EqWizardSession`, `OverlaySession`,
  `TimeAlignmentSession`, `RecordSettingsSession`, `AutoSetupWizardSession`, `VirtualCrossoverAuditionSession`,
  `FirConstructorSession`, `LiveSpectrumSettingsSession`, `VirtualCrossoverJunctionTuneQuestion`,
  `DspProcessorSession`, the mode settings sessions, the calibration dialogs' sessions); the UI writes it and presents what
  readers return.
