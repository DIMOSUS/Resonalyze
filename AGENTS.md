...
- Buttons, checkboxes and radio buttons are `ReleaseClickButton` / `ReleaseClickCheckBox` / `ReleaseClickRadioButton`, never the WinForms types they derive from — in the designer as well as in code. `ButtonBase` raises `Click` only while `WindowFromPoint` at the release point still answers with the control's own handle, so any window overlapping that pixel (a tooltip, above all — measured: 20 of 20 clicks lost) takes the click silently while the control still paints its press. A menu is then opened through `DropDownMenu`, which posts the show clear of the mouse message and guards it against the focus change opening it causes.
