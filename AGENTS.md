...
- **State gets a UI-free owner before features are written against it.** Once a second feature reads a tool's data,
  the data moves into a plain type (`VirtualCrossoverSession`, `AnalyzerDocument`, `LiveSpectrumSession`, `EqWizardSession`, `OverlaySession`); the UI writes it and presents what
  readers return.
