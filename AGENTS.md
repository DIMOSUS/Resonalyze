...
- Buttons, checkboxes and radio buttons are `ReleaseClickButton` / `ReleaseClickCheckBox` / `ReleaseClickRadioButton`, never the WinForms types they derive from — in the designer as well as in code. WinForms puts a `WindowFromPoint` ownership check in its mouse-release click path — one of several conditions, alongside cancelled validation and `ButtonBase`'s own press/capture state — so any window overlapping that one pixel (a tooltip, above all — measured: 20 of 20 clicks lost) takes the click silently while the control still paints its press. `ReleaseClick*` works around that hit-test failure and only it: the hit test is re-run before a click is manufactured, so a click WinForms withheld for a reason of its own stays withheld. A menu is then opened through `DropDownMenu`, which posts the show clear of the mouse message and guards it against the focus change opening it causes.
