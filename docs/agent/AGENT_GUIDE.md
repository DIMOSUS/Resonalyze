...
| `tuneJunction` | The crossover engine for a finished tune: ONE junction, both facing edges, each candidate scored on the pair's coherent sum after re-aligning the upper block's delay and polarity for it, everything else kept. Name steeper slopes in `slopes` when a ragged junction phase over a wide overlap is the problem. Its report also says which shift of the upper block each reading assumed: the tune writes only the crossover, so when that shift is not negligible the junction wants realigning — ask for `runAutoDelay` in the NEXT reply, once the user has copied a package and you have read what the tune actually did. (An import can run both; the reason to separate them is that you cannot judge the second while the first is unread.) |
