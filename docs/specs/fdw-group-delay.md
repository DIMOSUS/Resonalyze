...
## 8a. Ревью GPT по PR #186 (владелец переслал; пятый и шестой коммиты)

1. **Свежая установка стартовала на Fixed** — принято, блокер. Первый запуск
   идёт через `LoadOrDefault` → новый `MeasurementSettingsFile` → у поля схемы
   не было инициализатора → `ApplyTo` читал null как старый файл. Теперь у
   поля инициализатор FDW, старые файлы отличаются версией схемы (13, миграция
   `< 13` → Fixed, как у фазы версией 7); тесты — через реальный путь загрузки:
   несуществующий файл → FDW, файл версии 12 без поля → Fixed.
2. **Психоакустика** — то же, что нашёл Codex, закрыто коммитом ранее.
3. **FDW-excess ≠ классический all-pass** — принято как вопрос, отвечено
   замером, а не доказательством: цепочки PEQ Q 2/5/10 ±6 дБ на 100/1к/5к,
   гейты фазы (0,5/4/1,5), GD (0,5/10/3) и VDSP (1/3/12), циклы 4/6/8 — max
   |excess − bulk| ≤ 0,054 мс (p95 ≤ 0,006), у Fixed ≤ 0,003; LR24 80/3к и
   BW48 300/2,5к+PEQ — расхождение FDW vs Fixed ≤ 0,05 мс, а большие значения
   (1–14 мс у 101 Гц) одинаковы под ОБОИМИ окнами: это гейт режет звон HP, не
   FDW. Закреплено тестами `MinimumPhasePeqChain_ReadsNearZeroExcessUnderFdw`
   (7 комбинаций, допуск 0,06) и `Crossover_ExcessUnderFdw_IsTheFixedGatesExcess`.
   Проба оставлена на окне проекта; PROTOCOL, conventions и REFERENCE называют
   величину windowed reading с этой оговоркой и убирают «a group delay is a
   phase slope» (п. 5).
4. **«Пара» по моде и циклам** — принято: у вкладок разные гейты по умолчанию
   (0,5/4/1,5 против 0,5/10/3), пара = один гейт + мода + циклы; в VDSP оба
   вида читают один гейт по построению. REFERENCE и тултип поправлены.
5. **PROTOCOL** — поправлен вместе с п. 3.
