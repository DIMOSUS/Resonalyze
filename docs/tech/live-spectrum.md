...
- The analysis reuses its per-frame arrays: `OverlapReframer` hands out one frame it overwrites at each hop, and
  ...
  is MathNet's own FFT scratch (about 40 KiB). The warm-up runs through the run's own buffers, so they exist before
  the driver starts, and the one place a frame is kept, the seed of the average, copies it.
