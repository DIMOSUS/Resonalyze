...
- `CaptureSessionId` is regenerated by an `Init` whose route, format, playback channel or sequence length
  differs from the last one; a reconfigure that repeats them (a history entry, New session, an unchanged
  Apply) keeps it. Spatially averaged captures are levelled
