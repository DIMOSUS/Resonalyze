...
side cost nothing on the other; it answers with that read's own extent, never with the one it remembered while shown,
which also spans the other side's dashed sum as it was then and would hold the scale to a level that side has left. A shown side with nothing to draw (no source, or an empty group view) takes the same
