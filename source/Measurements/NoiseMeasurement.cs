...
namespace Resonalyze
{
    ...
    public sealed class NoiseMeasurement : IDisposable
    {
        ...
        // Numbered as queued, so the reader finds a dropped block by the gap in the numbers, wherever the drop landed
        // relative to its own dequeue.
        private long nextSequenceIndex;
        ...
        private readonly record struct LiveSequence(float[][] Channels, long Index, long DiscontinuityGeneration);
        ...
        private object? captureSessionKey;
        ...
        /// <summary>Regenerated by an <see cref="Init"/> that changes the route or format; without an SPL anchor, spatial-average captures share one level offset only within one id (hardware gain changes are invisible to it).</summary>
            ...
            int wasapiBufferMilliseconds = 100)
        {
            ...
            // Every Init argument but the options and the duration; a reconfigure repeating them is the same session.
            object sessionKey = (
                sampleRate, bits, playbackChannel, sequenceLength, outputDeviceNumber, inputDeviceNumber, audioBackend,
                asioDriverName, asioInputChannelOffset, asioOutputChannelOffset, waveInputChannelOffset,
                waveLoopbackInputChannelOffset, asioLoopbackInputChannelOffset, wasapiCaptureEndpointId,
                wasapiRenderEndpointId, wasapiBufferMilliseconds);
            if (!sessionKey.Equals(captureSessionKey))
            {
        ...
        private void HandleFrame(AudioCaptureFrame frame)
        {
            ...
            Volatile.Read(ref sequenceWriter)?.TryWrite(new LiveSequence(
                frame.Channels,
                Interlocked.Increment(ref nextSequenceIndex),
                Interlocked.Read(ref discontinuityGeneration)));
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                if (LiveSequenceContinuity.Breaks(
                        previousIndex,
                        queued.Index,
                        consumedDiscontinuityGeneration,
                        queued.DiscontinuityGeneration))
                {
