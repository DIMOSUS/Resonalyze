...
/// <summary>The FIR Constructor's rules read a <c>FirConstructorSession</c>, and the panel binds the controls to them. A
/// static or a nested type the panel exposes is a rule a test reaches only through the panel.</summary>
public sealed class FirConstructorPanelBoundaryTests
{
    ...
    {
        List<string> exposed = typeof(FirConstructorPanel)
            .GetMembers(Declared)
