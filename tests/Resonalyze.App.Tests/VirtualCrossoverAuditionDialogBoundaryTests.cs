...
/// <summary>The audition's rules read a <c>VirtualCrossoverAuditionSession</c>, and the dialog binds the controls to them. A
/// static or a nested type the dialog exposes is a rule a test reaches only through the dialog.</summary>
public sealed class VirtualCrossoverAuditionDialogBoundaryTests
{
    ...
    {
        List<string> exposed = typeof(VirtualCrossoverAuditionDialog)
            .GetMembers(Declared)
