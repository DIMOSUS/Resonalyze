...
public sealed class VirtualCrossoverGatePlacementWarningTests
{
    ...
    public void AGateInheritedFromAnotherSessionReadsAsOpeningLate()
    {
        ...
        Assert.Null(GatePlacementVerdict.JudgeCut(
    ...
    public void AChannelInsideTheFadeOutIsNotReportedAsMissing()
    {
        ...
        Assert.Null(GatePlacementVerdict.JudgeCut(
            ...
            placementLossDb: -60.0, ownArrivalLossDb: -30.0));
        Assert.Equal(
            GateCutKind.ClosesBeforeArrival,
            GatePlacementVerdict.JudgeCut(
    ...
    public void TheWarningNamesTheSideTheOffsetAndTheChannelsItCuts()
    {
        ...
        {
            string warning = PassatVerdict(pinned: true).FormatWarning();
    ...
    public void AWindowThatClosedTooEarlySaysThatInsteadOfBlamingTheTail()
    {
        RunWithInvariantCulture(() =>
        {
            var verdict = new GatePlacementVerdict(
                OffsetMs: 4.125,
                PlateauMs: 4.0,
                RightMs: 1.5,
                Pinned: false,
                RightSide: false,
                Cut: new List<GateCutChannel>
                {
                    new("D", 19.108, GateCutKind.ClosesBeforeArrival, -2.2)
                });

            string warning = verdict.FormatWarning();
            Assert.Contains("is over before D arrives (19.11 ms)", warning);
            Assert.Contains("that curve holds none of it", warning);

            string detail = verdict.FormatDetail();
    ...
    public void APinnedGateIsToldToGoBackToAuto()
    {
        RunWithInvariantCulture(() =>
        {
            string detail = PassatVerdict(pinned: true).FormatDetail();
    ...
    public void AnAutoGateThatStillOpensLateIsToldToWidenTheShoulderInstead()
    {
        RunWithInvariantCulture(() =>
        {
            string detail = PassatVerdict(pinned: false).FormatDetail();
    ...
    public void AWindowHoldingNoneOfAChannelSaysSoInsteadOfPrintingInfinity()
    {
        RunWithInvariantCulture(() =>
        {
            var verdict = new GatePlacementVerdict(
                ...
                Cut: new List<GateCutChannel>
                {
                    new("D", 4.119, GateCutKind.OpensAfterArrival,
    ...
    private static GatePlacementVerdict PassatVerdict(bool pinned) =>
        new(
            OffsetMs: 15.06,
            PlateauMs: 50.0,
            RightMs: 20.0,
            ...
            Cut: new List<GateCutChannel>
            {
                new("B", 5.967, GateCutKind.OpensAfterArrival, 1.0),
                new("C", 4.101, GateCutKind.OpensAfterArrival, 9.9),
                new("D", 4.119, GateCutKind.OpensAfterArrival, 15.2)
            });
}
